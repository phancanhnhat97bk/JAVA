...
<head>
		...
		<meta charset="utf-8" />
		<title>Thêm danh sách bài hướng dẫn</title>
	...
	<!-- End Menu -->
				...
				<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<c:if test="${guidelineroleid==1}">
								<li class="active">Bài hướng dẫn ngữ pháp</li>
							</c:if>
							<c:if test="${guidelineroleid==2}">
								<li class="active">Bài hướng dẫn nghe</li>
							</c:if>
						</ul><!-- /.breadcrumb -->
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
							<c:if test="${msglistguidelinemanage!=null}">
								<h4 class="pink">
									<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i>
									<a class="green" data-toggle="modal"> ${msglistguidelinemanage} </a>
								</h4>
								...
								<div class="row">
									<div class="col-xs-12">
										<table id="simple-table" class="table  table-bordered table-hover">
											...
											<tbody>
												<c:forEach items="${listguidelinemanage}" var = "list">
													<tr>
													<td class="center">
														${list.guidelineid}
													</td>
													...
													<td class="center">
														<a class="red" href="Deleteguideline?guidelineid=${list.guidelineid}&guidelineroleid=${guidelineroleid}">
															<i class="ace-icon fa fa-trash-o bigger-130"></i>
														</a>
													</td>
													<td class="center">
														<a class="green" href="GuidelinecontentForward?guidelineid=${list.guidelineid}&guidelineroleid=${guidelineroleid}">
										...
										<div>
											<ul class="pagination">
												...
												<c:if test="${numberpage ==1 && numberpage != maxpageid}">
													...
													<li><a href="Listguidelinemanage?pageid=${numberpage +1}&guidelineroleid=${guidelineroleid}">Next</a></li>
												</c:if>
												
												<c:if test="${numberpage == maxpageid  && numberpage !=1}">
													<li><a href="Listguidelinemanage?pageid=${numberpage -1}&guidelineroleid=${guidelineroleid}">Prev</a></li>
								...
								<div class="row">
									...
									<c:if test="${guidelineroleid==1}">
										<button type="button" class="btn btn-white btn-warning btn-bold" data-toggle="modal" data-target="#myModal">
											...
											Thêm tiêu đề bài hướng dẫn ngữ pháp
										</button>
									</c:if>
									<c:if test="${guidelineroleid==2}">
										<button type="button" class="btn btn-white btn-warning btn-bold" data-toggle="modal" data-target="#myModal">
											<i class="ace-icon fa fa-floppy-o bigger-120 blue"></i>
											Thêm tiêu đề bài hướng dẫn nghe
										</button>
									</c:if>
		...
		<div id="myModal" class="modal fade" role="dialog">
		  <div class="modal-dialog">
			...
			<form action="Insertguidelinename?guidelineroleid=${guidelineroleid}&userid=${userid}" method="POST">
		    <!-- Modal content-->
		    <div class="modal-content">
		      <div class="modal-header">
		        <button type="button" class="close" data-dismiss="modal">&times;</button>
		        <c:if test="${guidelineroleid==1}">
		        	<h4 class="modal-title">Thêm tiêu đề bài hướng dẫn ngữ pháp</h4>
		        </c:if>
		        <c:if test="${guidelineroleid==2}">
		        	<h4 class="modal-title">Thêm tiêu đề bài hướng dẫn nghe</h4>
		        </c:if>
		      </div>
		      ...
		      <div class="col-xs-12">
		        <div class="form-group">
					...
					<div class="col-sm-9">
						<c:if test="${guidelineroleid==1}">
							<input type="text" id="form-field-1-1" placeholder="Nhập tiêu đề bài hướng dẫn ngữ pháp" class="form-control" name ="guidelinename"/>
						</c:if>
						<c:if test="${guidelineroleid==2}">
							<input type="text" id="form-field-1-1" placeholder="Nhập tiêu đề bài hướng dẫn nghe" class="form-control" name ="guidelinename"/>
						</c:if>
					</div>
		      ...
		      <div class="col-xs-12">
		        <div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Cấp độ : </label>
					<div class="col-sm-9">
							<select name="level">
